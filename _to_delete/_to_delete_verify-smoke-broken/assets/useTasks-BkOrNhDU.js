...
import { a as isTaskUpcoming, c as normalizePostponeHistory, l as normalizeStartDate, n as applyTaskUpdates, o as normalizeEnergyLevel, s as normalizePlannedDate, u as shiftStartDateForDeadline } from "./taskFields-B8eA_8sb.js";
import { i as deadlineForBucket } from "./buckets-CeS2d1pg.js";
import { a as reminderKey, l as nextOccurrence } from "./reminders-Dz29wQZM.js";
import { useEffect, useState } from "react";
//#region src/hooks/useTasks.js
var STORAGE_KEY = "tidyline:tasks";
var UNDO_MS = 6e3;
...
function useTasks() {
	...
	useEffect(() => {
		const interval = window.setInterval(() => {
			...
			setTasks((current) => {
				return current.some((task) => task.plannedDate && task.plannedDate < today || task.status === "waiting" && task.followUpDate && task.followUpDate <= today) ? current.map((task) => {
					const releaseWaiting = task.status === "waiting" && task.followUpDate && task.followUpDate <= today;
					return {
						...task,
						plannedDate: task.plannedDate && task.plannedDate < today ? null : task.plannedDate,
	...
	function addTask({ title, deadline, reminders, tags = [], recurrence = null, notes = "", checklist = [], links = [], attachments = [], location = "", duration = null, startDate = null, energyLevel = null, scheduledStart = null, archived = false, status = "active", waitingFor = "", followUpDate = null, plannedDate = null }) {
	...
	function updateTask(id, updates, source = "edit") {
		setTasks((current) => current.map((task) => task.id === id ? applyTaskUpdates(task, updates, source) : task));
	}
	...
	function completeTask(id) {
		...
		let next = tasks.map((task) => task.id === id ? {
			...task,
			done: true,
	...
	function setDeadline(id, deadline, source = "calendar", extraUpdates = {}) {
		...
		const updated = applyTaskUpdates(target, {
			...extraUpdates,
			deadline
		}, source);
		if (updated === target) return;
		commit("Task rescheduled", tasks.map((task) => task.id === id ? updated : task));
	}
	function moveTaskToBucket(id, bucketKey, bucketOrder) {
		setDeadline(id, deadlineForBucket(bucketKey, /* @__PURE__ */ new Date(), bucketOrder), "drag", { plannedDate: null });
	}
	function promoteSomeday(id, deadline) {
		if (!deadline) return;
		patch(id, (task) => task.deadline ? {} : {
			deadline,
			originalDeadline: deadline
		});
	}
	...
	function addAttachment(id, attachment) {
		patch(id, (task) => ({ attachments: [...task.attachments, {
			id: crypto.randomUUID(),
			...attachment
		}] }));
	}
	function removeAttachment(id, attachmentId) {
		patch(id, (task) => ({ attachments: task.attachments.filter((entry) => entry.id !== attachmentId) }));
	}
	function bulkComplete(ids) {
		const set = new Set(ids);
		const stamp = (/* @__PURE__ */ new Date()).toISOString();
		commit(`${ids.length} task${ids.length === 1 ? "" : "s"} completed`, tasks.map((task) => set.has(task.id) && !task.done ? {
			...task,
			done: true,
			completedAt: stamp
		} : task));
	...
	function bulkArchive(ids) {
		const set = new Set(ids);
		commit(`${ids.length} task${ids.length === 1 ? "" : "s"} archived`, tasks.map((task) => set.has(task.id) ? {
			...task,
			archived: true
		} : task));
	}
	function bulkDelete(ids) {
		const set = new Set(ids);
		commit(`${ids.length} task${ids.length === 1 ? "" : "s"} deleted`, tasks.filter((task) => !set.has(task.id)));
	}
...
//#endregion
export { useTasks as n, normalizeTask as t };
