...
| PLAN-01 | High | In progress | Natural-language due times are preserved, displayed before save, and used for relative reminders. |
| PLAN-02 | Medium | In progress | Calendar workload redistribution changes the proposed deadlines after confirmation. |
| PLAN-03 | Medium | In progress | Quick Add priority and full-form `plan today` have end-to-end behavior, or their unsupported promises are removed. |
| PLAN-04 | Medium | Complete | Completion-day analytics use local dates consistently; recurrence behavior has documented catch-up and month-end rules. |
| QUAL-01 | Medium | In progress | Browser-level checks cover data recovery, sync interruption, notifications, touch, keyboard, and responsive layouts. |
| QUAL-02 | Low | Complete | Remove tracked generated output, align README and preview with the current product, and correct the page title. |
...
- REL-04: Clean account views refetch on focus and every minute; a per-account browser cache updates other open tabs. Local task migration offers an additive merge even when the account has tasks. The migration prompt appeared correctly during a live Google sign-in on 2026-09-19. Normal task edits and deletes now use an atomic database revision check. If another device has already changed the task, its newer account version remains under the original ID and the stale local edit is saved under a new ID with a `conflict copy` label; a stale delete restores the newer account version. Later queued writes for the stale ID are folded into that copy. Apply `20260923_task_revisions.sql`. Automated checks cover the conflict-preservation queue transition; a two-device live test remains before completion.
...
- WEB-04: All six modal surfaces now share focus entry, Tab and Shift+Tab containment, Escape handling, background-scroll locking, and focus restoration. The dialog role now sits on the actual modal panel, and Quick Add scrolls within the viewport at high zoom or on short screens. The Command Palette passed an in-browser keyboard check for initial focus, forward and reverse focus containment, Escape handling, and scroll-lock restoration on 2026-09-20. Screen-reader, 200% zoom, and mobile device-matrix checks remain before completion.
- PLAN-01: Quick Add now preserves explicit due times, previews them before save, passes them into the full form, and stores them separately from the due date. Creation and edit forms expose an optional time, task surfaces display it, and both open-page and background relative reminders resolve from it while date-only tasks retain the 09:00 fallback. Apply `20260920_deadline_times.sql` and redeploy `send-reminders`; a live timed reminder remains before completion.
- PLAN-02: Calendar now receives the real batch rescheduler, so confirming a redistribution updates local task state, postponement history, the durable account queue, and Supabase sync. The planner excludes explicitly planned or timed tasks and refuses to overload a target day. Automated checks cover proposal application and protected tasks; a browser confirmation check remains before completion.
- PLAN-03: Quick Add priority now survives direct creation and the full-form handoff, syncs through Supabase, appears on tasks, can be edited, filtered, sorted, and reused in templates, and protects high-priority work from automatic redistribution. The full form now preserves and exposes `plan today`, rejects future-start or waiting combinations, and stores only actionable plans. Apply `20260920_task_priority.sql`; live browser creation, editing, filtering, and signed-in reload checks remain before completion.
- PLAN-04: Completion history, today's completion count, and the activity heatmap now group `completedAt` timestamps by the user's local calendar day. The heatmap uses the actual completion date instead of the deadline. Recurring tasks skip missed instances after overdue completion, retain monthly and yearly calendar anchors through short months and leap years, and behave the same during bulk completion. Open-page and background recurring reminders use local subscription dates and the same month-end rule. Automated checks cover local completion grouping, catch-up, January 31, February 29, and time-zone-aware reminder anchors. The policy is recorded in `RECURRENCE_RULES.md`. Completed 2026-09-20.
- QUAL-01: An isolated in-app browser run on 2026-09-20 confirmed guest Quick Add persistence across reload and Command Palette keyboard containment. Settings accurately exposed blocked browser permission and missing background-push server setup. Existing automated checks cover recovery, durable sync queue replay, service-worker offline fallback, reminder rules, and modal mounting. Physical touch devices, ordinary-browser offline reload, closed-page delivery, screen readers, 200% zoom, and two-device sync remain release checks.
- QUAL-02: Tracked smoke/parser build output, unused starter assets, and the stale product screenshot were removed. Generated test folders are ignored, the README no longer presents an outdated interface, and the page title remains TidyLine. Completed 2026-09-20.

The configured Supabase backend has passed sign-in and single-browser persistence checks. Apply the task-revision migration before the next signed-in test. Background push deployment, two-device convergence, and real mobile notification behavior remain unverified.
