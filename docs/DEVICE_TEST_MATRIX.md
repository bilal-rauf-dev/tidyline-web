...
2026-09-20, Codex in-app browser on Windows, isolated local guest origin: Quick Add created a task and the task survived a Board-route reload. The Command Palette placed focus in its search field, kept both Tab and Shift+Tab inside the dialog, closed with Escape, and restored page scrolling. Settings accurately reported that open-page notification permission was blocked and that background delivery still required server setup. This environment could not change viewport size or validate real service-worker offline reload, notification permission, background delivery, touch, or screen-reader behavior; those checks remain open on ordinary browsers and physical devices.
