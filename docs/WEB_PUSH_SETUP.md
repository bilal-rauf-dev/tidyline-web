...
4. `supabase_migrations/20260920_task_priority.sql`
5. `supabase_migrations/20260923_task_revisions.sql`
6. `supabase_migrations/20260920_web_push.sql`

The deadline-time migration lets relative reminders use the time the user entered. The priority migration keeps Quick Add and task-form priority choices in sync across devices. The push migration stores one endpoint per browser, moves an endpoint safely when a shared browser changes accounts, records delivery and retry state, and keeps all browser reads behind row-level security.
